...
让 AI **快速搞懂代码项目**：只读盘点仓库结构、追踪核心流程、梳理埋点事件，输出 Markdown 报告、交互式 HTML 指南，或针对指定模块的聚焦分析。
...
| 🌐 **深层分析** | Phase 1 骨架卡片 + Phase 2 `project-guide/` 目录 | 两阶段交互式 HTML 指南：项目骨架 → 全面深入 |
...
Skill 激活后先执行只读检测：项目规模、已有分析缓存、Git 可用性。

- 如果用户已指定浅层、深层、聚焦、输出格式或目标模块，直接按用户要求执行，并说明依据。
- 如果用户未指定模式，按规模和缓存推荐默认模式。
- 只有在模式差异会显著影响耗时、写入范围或输出格式时，才向用户确认一次。
...
| 超大型 | 1000+ | 先浅层概览，再按模块聚焦 |

已有 `project-guide/` 缓存时，支持增量更新；变更很少时自动增量，变更很大或 lockfile/manifest 变化时自动全量，中间区间再询问用户。
...
3. 告诉 AI 项目路径和期望模式；未指定模式时由 Prompt 按规模推荐
...
- 标记阻碍启动的严重风险，其余只描述现状
- 分析深度随项目规模自适应调整
- 避免未验证的承诺式表述；区分已观察、文档声明、运行验证、推断和待确认
