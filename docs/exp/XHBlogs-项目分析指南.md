...
>
> **补充说明（2026-08-05）**：本项目（EverlastingDemo）已决定将「说说 /moments」「杂谈 /chatter」「文章 /posts」整合为统一「杂谈」模块（`notes/` 单目录 + kind 字段 + 本地编辑器），相关方案见《EverlastingDemo-内容整合企划书-杂谈统一模块.md》。本文档保留为对参考项目 XHBlogs 的历史分析，与落地形态不一致之处以企划书与整合指南为准。
